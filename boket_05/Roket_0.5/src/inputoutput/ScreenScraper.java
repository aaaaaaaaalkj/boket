...
public final class ScreenScraper {
	...
	private void stackRecognition(final Pos logo) {
		...
		for (int i = 0; i < NUM_SEATS; i++) {
			...
			@NonNull
			Pos p = positions[i];
			Optional<Double> opt = ocrRealMoney(logo.plus(p), ref,
					4, 7, 4, indicators);
			...
			if (opt.isPresent()) {
				...
				opt = ocrRealMoney(logo.plus(p), ref,
						4, 7, 4, indicators);
	...
	@SuppressWarnings("null")
	private void activePlayers(final Pos logo) {
	...
	void searchButton(final Pos logo) {
	...
	@SuppressWarnings("null")
	private void postRecognition(final Pos logo) {
	...
	private Optional<Double> ocr(final Pos logo, final Pos pos) {
		...
		if (REAL_MONEY) {
			...
			return ocrRealMoney(logo.plus(pos), ref, 4, 6, 3, indicators);
		}
		else
	...
	@Nullable
	private Integer recognizeFirstLetter(final Pos logo, final Pos pos,
	...
	private Double ocr_play_money(final Pos logo, final Pos pos) {
		// r=255,g=246,b=207
		Color ref = new Color(16774863);
	...
	private void recognizeBrownButtons(final Pos logo) {
	...
	private boolean checkBrownButton(final Pos logo, final Pos pos) {
